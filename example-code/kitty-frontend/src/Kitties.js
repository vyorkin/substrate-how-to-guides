...
export default function Kitties (props) {
  ...
  const fetchKittyCnt = () => {
    api.query.kitties.allKittiesCount(async (cnt) => {
      const cntNum = cnt.toNumber();
      setKittyCnt(cntNum);

      console.log(`kitty count: ${cntNum}`);

      // Here we fetch all the kitty hash
      const kittyIndices = [...Array(cntNum).keys()];
      api.query.kitties.allKittiesArray.multi(kittyIndices, (keys) => {

        console.log('All kitty keys:', keys.map(key => key.toHuman()));

        // Fetching all kitties based on the kitty key;
        api.query.kitties.kitties.multi(keys, kittyInfo => {
          kittyInfo.forEach((kitty, ind) => {
            console.log(`Kitty ${ind}| id: ${kitty.id}, dna: ${kitty.dna}, price: ${kitty.price}, gender: ${kitty.gender}`);
          });
        });
      });
    });
